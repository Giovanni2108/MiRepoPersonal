...
def fetch_data():
    ...
    try:
        url = (
            ...
            "?latitude=13.6893&longitude=-89.1872"
...
def create_line_chart(horas, temps):
    ...
    ax.plot(horas, temps, color="darkorange", linestyle="-", marker="o", markersize=3, linewidth="2")
    ax.set_title("Temperatura en San Salvador (línea)", fontsize=14, fontweight='bold')
    ax.set_xlabel("Hora")
    ax.set_ylabel("°C")
...
def create_bar_chart(horas, temps):
    ...
    colors = cm.viridis(norm(temps))
    bars = ax.bar(horas, temps, color=colors)
    ax.set_title("Temperatura en San Salvador (barras)", fontsize=14, fontweight='bold')
    ax.set_xlabel("Hora")
    ax.set_ylabel("°C")
    ax.tick_params(axis="x", rotation=45)
