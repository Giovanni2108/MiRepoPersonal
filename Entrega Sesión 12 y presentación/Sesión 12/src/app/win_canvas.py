...
def fetch_data():
    ...
    try:
        url = (
            ...
            "?latitude=34.35&longitude=58.22"
...
def create_line_chart(horas, temps):
    ...
    ax.plot(horas, temps, linestyle="-", marker="s", markersize=3,color='orange')
    ax.set_title("Temperatura en Buenos Aires (línea)",color='blue')
    ax.set_xlabel("Hora")
    ax.set_ylabel("°C")
...
def create_bar_chart(horas, temps):
    ...
    ax.set_title("Temperatura en Buenos aires (barras)",color='blue')
    ax.set_xlabel("Hora")
    ax.set_ylabel("°C")
    ax.tick_params(axis="x", rotation=45)
    fig.tight_layout()
